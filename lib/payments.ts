...
export async function handleSubscriptionDeleted({
    ...
    try {
        ...
        await sql`UPDATE users SET status='cancalled' WHERE customer_id=${subscription.customer}`;
...
export async function handleCheckoutSessionCompleted({
  ...
  // Make sure session has line_items expanded
  const priceId = session.line_items?.data[0]?.price?.id;

  if (customer && "email" in customer && customer.email && priceId) {
...
async function createOrUpdateUser({
  ...
  try {
    ...
    // Use the email passed as argument, not empty string
